...
PYTHONPATH=. python workspace/preprocess/build_dataset.py \
  --human-json workspace/dataset/source/data/poker_hands_train.json \
  --zenodo-human-json workspace/dataset/source/data/zenodo_v3/poker_hands_zenodo_test.json \
  --zenodo-human-json workspace/dataset/source/data/zenodo_v3/poker_hands_zenodo_train.json \
  --bot-json workspace/dataset/source/data/bot/poker_hands_bot_train.jsonl \
  --bot-json-fraction 0.002 \
  --target-bot-ratio 0.5 \
  --out workspace/dataset/raw_dataset/concat \
  ...
  --miner-feature-schema strict \
  --format parquet \
  --seed 42 \
  --val-size 0.2 \
  --cpu-friendly \
  --human-merge-max-rows 300000 \
  --shard-size 100 \
  --resume
...
def _load_hand_objects_any(path: Path) -> List[Dict[str, Any]]:
    """
    Load hands from:
      - JSON array
      - JSONL / JSONL.GZ (one object per line)
      - JSON object with key 'hands' containing a list
    """
    p = Path(path).expanduser().resolve()
    if not p.is_file():
        raise DataValidationError(f"Hand source not found: {p}")
    if _human_source_is_jsonl(p):
        return _load_jsonl_hand_objects(p)
    if p.suffix.lower() == ".gz":
        with gzip.open(p, "rt", encoding="utf-8") as f:
    ...
    if isinstance(data, dict):
        ...
        if isinstance(hands, list):
            data = hands
    if not isinstance(data, list):
        raise DataValidationError(f"{p}: expected list/JSONL or dict with 'hands'.")
    out: List[Dict[str, Any]] = []
    for i, item in enumerate(data):
        if not isinstance(item, dict):
            raise DataValidationError(f"{p}: item {i} is not an object.")
...
def _iter_json_array_objects_stream(path: Path) -> Iterator[Dict[str, Any]]:
    """Stream objects from a top-level JSON array without loading full file."""
    ...
    with path.open("r", encoding="utf-8") as f:
        ...
        while first and first.isspace():
            first = f.read(1)
        if first != "[":
            raise DataValidationError(f"{path}: expected top-level JSON array for streaming.")
        buf = ""
        done = False
        eof = False
        while not done:
            ...
            if i > 0:
                buf = buf[i:]
            if eof and not done:
                raise DataValidationError(f"{path}: malformed/truncated JSON array.")


def _iter_hand_objects_any(path: Path) -> Iterator[Dict[str, Any]]:
    p = Path(path).expanduser().resolve()
    if not p.is_file():
        raise DataValidationError(f"Hand source not found: {p}")
    if _human_source_is_jsonl(p):
    ...
    # For .json(.gz): fallback to full load only for .gz or non-array.
    if p.suffix.lower() == ".gz":
        with gzip.open(p, "rt", encoding="utf-8") as f:
            data = json.load(f)
    ...
    # Plain .json: stream top-level array for memory safety.
    yield from _iter_json_array_objects_stream(p)


def _merge_multiple_human_sources(
    *,
    ...
    if not sources:
        raise DataValidationError("No human sources provided to merge.")
    source_stats: List[Dict[str, Any]] = []
    rng = random.Random(int(seed))

    if max_total_rows is None or int(max_total_rows) <= 0:
        rows_all: List[Dict[str, Any]] = []
        for src in sources:
            rows = _load_hand_objects_any(src)
            rows_all.extend(rows)
            source_stats.append({"path": str(src), "rows": len(rows)})
        if not rows_all:
            raise DataValidationError("Merged human pool is empty.")
        rng.shuffle(rows_all)
        _write_json_array(out_path, rows_all)
        return out_path, {
            "seed": int(seed),
            "sources": source_stats,
            "merged_total": len(rows_all),
            "out_path": str(out_path),
            "sampled_cap": None,
        }

    cap = int(max_total_rows)
    reservoir: List[Dict[str, Any]] = []
    seen = 0
    for src in sources:
        src_count = 0
        for row in _iter_hand_objects_any(src):
            src_count += 1
            seen += 1
            if len(reservoir) < cap:
                ...
                if j <= cap:
                    reservoir[j - 1] = row
        source_stats.append({"path": str(src), "rows_seen": src_count})
    if not reservoir:
        raise DataValidationError("Merged human pool is empty.")
    rng.shuffle(reservoir)
    _write_json_array(out_path, reservoir)
    return out_path, {
        "seed": int(seed),
        "sources": source_stats,
        "merged_total": len(reservoir),
        "rows_seen_total": int(seen),
        "out_path": str(out_path),
        "sampled_cap": int(cap),
    }


def _sample_bot_hands(
    *,
    bot_sources: Sequence[Path],
    fraction: float,
    seed: int,
    min_needed_rows: int = 0,
) -> List[Dict[str, Any]]:
    if not bot_sources:
        return []
    if fraction <= 0.0:
    ...
    if fraction >= 1.0:
        return all_rows
    take = max(1, int(round(len(all_rows) * fraction)))
    sampled = all_rows[:take]
    # Policy:
    # - Large source: keep fraction sample (e.g. 0.2%).
    # - Small source: if sampled pool is too small for planned external-bot chunk budget,
    #   use full source to avoid excessive repetition from a tiny sample.
    if int(min_needed_rows) > 0 and len(sampled) < int(min_needed_rows):
        return all_rows
    return sampled


def _build_labeled_bot_chunks_from_hands(
    *,
    bot_hands: Sequence[Dict[str, Any]],
    chunk_count: int,
    min_hands_per_chunk: int,
    max_hands_per_chunk: int,
    seed: int,
) -> List[Dict[str, Any]]:
    if not bot_hands or chunk_count <= 0:
        return []
    rng = random.Random(int(seed))
    if min_hands_per_chunk <= 0 or max_hands_per_chunk < min_hands_per_chunk:
        raise DataValidationError("Invalid min/max hands per chunk for bot chunking.")
    pool = list(bot_hands)
    if len(pool) < min_hands_per_chunk:
        # pad from existing rows so chunker can proceed
        pool.extend(rng.choices(pool, k=min_hands_per_chunk - len(pool)))
    rng.shuffle(pool)
    out: List[Dict[str, Any]] = []
    cursor = 0
    n = len(pool)
    for _ in range(chunk_count):
        size = rng.randint(min_hands_per_chunk, max_hands_per_chunk)
        if cursor + size > n:
            rng.shuffle(pool)
            cursor = 0
        hands = pool[cursor : cursor + size]
        cursor += size
        out.append({"hands": hands, "is_bot": True})
    return out


def _rebalance_labeled_chunks(
    labeled_chunks: List[Dict[str, Any]],
    *,
    target_bot_ratio: Optional[float],
    seed: int,
) -> List[Dict[str, Any]]:
    if target_bot_ratio is None:
        return labeled_chunks
    if not (0.0 < float(target_bot_ratio) < 1.0):
        raise DataValidationError("--target-bot-ratio must be in (0,1).")
    bots = [c for c in labeled_chunks if bool(c.get("is_bot", False))]
    humans = [c for c in labeled_chunks if not bool(c.get("is_bot", False))]
    if not bots or not humans:
        raise DataValidationError("Cannot rebalance classes: one class is empty.")
    rng = random.Random(int(seed))
    rng.shuffle(bots)
    rng.shuffle(humans)
    bot_ratio = float(target_bot_ratio)
    # choose max feasible total without replacement
    total = min(int(len(bots) / bot_ratio), int(len(humans) / (1.0 - bot_ratio)))
    if total < 2:
        raise DataValidationError("Insufficient chunks to satisfy target bot ratio.")
    n_bot = max(1, min(len(bots), int(round(total * bot_ratio))))
    n_human = max(1, min(len(humans), total - n_bot))
    out = bots[:n_bot] + humans[:n_human]
    rng.shuffle(out)
    return out
...
def chunks_to_dataframe(
    ...
    *,
    ...
    print(
        f"[featurize] start chunks={total} skip_sanitize={skip_sanitize}",
        flush=True,
    )
    for i, ch in enumerate(labeled_chunks, start=1):
        hands = ch.get("hands") or []
        row = aggregate_chunk_from_hands(hands, skip_sanitize=skip_sanitize)
...
def _miner_feature_schema_hash() -> str:
    cols = list(miner_servable_feature_names())
    payload = json.dumps(cols, sort_keys=False, separators=(",", ":"))
...
def merge_shards_and_split(
    *,
    ...
    meta: Dict[str, Any] = {
        ...
        "miner_feature_schema_hash": _miner_feature_schema_hash(),
    }
...
def build_sharded_training_dataset(
    *,
    ...
    miner_feature_schema: MinerFeatureSchemaMode = "strict",
    extra_bot_jsons: Optional[Sequence[Path]] = None,
    bot_json_fraction: float = 0.0,
    target_bot_ratio: Optional[float] = None,
    log_every_shards: int = 1,
    skip_sanitize: bool = False,
) -> Tuple[Path, Path, Dict[str, Any]]:
    """
    ...
    for shard_idx in range(n_shards):
        ...
        if resume and shard_path.exists():
            ...
            if int(log_every_shards) > 0 and ((shard_idx + 1) % int(log_every_shards) == 0 or (shard_idx + 1) == n_shards):
                elapsed = time.perf_counter() - t_all_start
                print(
                    f"[progress] shard {shard_idx + 1}/{n_shards} (resume-skip) elapsed={elapsed:.1f}s",
                    flush=True,
                )
            continue
        ...
        extra_bot_chunks: List[Dict[str, Any]] = []
        if extra_bot_jsons:
            # external bot chunks capped at 20% of shard chunk_count
            add_cc = max(0, int(round(sub_cc * 0.2)))
            min_needed_bot_rows = max(1, add_cc * max(1, int(mn)))
            sampled_bot_hands = _sample_bot_hands(
                bot_sources=extra_bot_jsons,
                fraction=float(bot_json_fraction),
                seed=sub_seed,
                min_needed_rows=min_needed_bot_rows,
            )
            extra_bot_chunks = _build_labeled_bot_chunks_from_hands(
                bot_hands=sampled_bot_hands,
                chunk_count=add_cc,
                min_hands_per_chunk=mn,
                max_hands_per_chunk=mx,
                seed=sub_seed + 7,
            ...
            if extra_bot_chunks:
                labeled_chunks.extend(extra_bot_chunks)
                stats["extra_bot_chunks"] = int(len(extra_bot_chunks))
                stats["extra_bot_hands_pool"] = int(len(sampled_bot_hands))
                stats["extra_bot_min_needed_rows"] = int(min_needed_bot_rows)

        labeled_chunks = _rebalance_labeled_chunks(
            labeled_chunks,
            target_bot_ratio=target_bot_ratio,
            seed=sub_seed + 13,
        )
        ...
        df = chunks_to_dataframe(
            labeled_chunks,
            progress_every=progress_every_chunks,
        ...
        if int(log_every_shards) > 0 and ((shard_idx + 1) % int(log_every_shards) == 0 or (shard_idx + 1) == n_shards):
            elapsed = time.perf_counter() - t_all_start
            shard_elapsed = time.perf_counter() - t_shard_start
            print(
                f"[progress] shard {shard_idx + 1}/{n_shards} done in {shard_elapsed:.1f}s "
...
def build_training_dataset(
    *,
    ...
    When *skip_sanitize* is True, hands are assumed to be already sanitized
    (e.g. from validator logs or gold data). Chunk counts/ratios follow ``preset``
    ...
    if extra_bot_jsons:
        # external bot chunks capped at 20% of requested chunk_count
        add_cc = max(0, int(round(cc * 0.2)))
        min_needed_bot_rows = max(1, add_cc * max(1, int(mn)))
        sampled_bot_hands = _sample_bot_hands(
            bot_sources=extra_bot_jsons,
            fraction=float(bot_json_fraction),
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Additional human JSON/JSONL source(s); can be provided multiple times.",
    )
    parser.add_argument(
        "--human-json-a",
        type=Path,
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="External bot hand source(s) (JSON/JSONL) to mix with generated bot chunks.",
    )
    parser.add_argument(
        "--bot-json-fraction",
        type=float,
        default=0.002,
        help="Fraction of rows to sample from --bot-json sources (default 0.002 = 0.2%%).",
    )
    parser.add_argument(
        "--target-bot-ratio",
        type=float,
        default=None,
        help="Optional final chunk-level bot ratio in (0,1), e.g. 0.5 for balanced classes.",
    )
    ...
    parser.add_argument(
        "--cpu-friendly",
        action="store_true",
        help="Reduce CPU pressure: fewer bot attempts/rounds and frequent logs.",
    )
    parser.add_argument(
        "--log-every-shards",
        type=int,
        default=1,
        help="Print sharded-progress log every N shards (default 1).",
    )
    parser.add_argument(
        "--human-merge-max-rows",
        type=int,
        default=0,
        help="Cap merged human pool rows (0 = unlimited). Useful to prevent OOM.",
    )
    parser.add_argument(
        "--skip-sanitize",
        action="store_true",
        help="Skip sanitize_hand_for_miner during feature extraction. Use when building "
        "from pre-sanitized data (validator logs, gold dataset). Preserves action types "
        "like 'other' that local sanitization would destroy.",
    )
    ...
    if args.cpu_friendly:
        # Best-effort caps for heavy native libs.
        ...
        # Soften expensive generation defaults unless the user already set tighter values.
        args.bot_candidate_attempts = max(1, min(int(args.bot_candidate_attempts), 2))
        args.bot_generation_rounds = max(1, min(int(args.bot_generation_rounds), 1))
        args.progress_every_chunks = max(25, min(int(args.progress_every_chunks), 50))
        args.log_every_shards = 1
        if int(args.human_merge_max_rows) <= 0:
            cc_eff, _mn_eff, mx_eff, hr_eff = _effective_chunk_params(
                cast(PresetName, args.preset),
                chunk_count=args.chunk_count,
                min_hands=args.min_hands,
                max_hands=args.max_hands,
                human_ratio=args.human_ratio,
            )
            # keep enough human pool for generation with headroom, but bounded.
            args.human_merge_max_rows = max(
                200_000,
                min(1_200_000, int(cc_eff * mx_eff * max(0.1, float(hr_eff)) * 2.0)),
            )
        print(
            "[cpu-friendly] enabled: capped bot complexity, set thread hints to 1, "
            f"human_merge_max_rows={int(args.human_merge_max_rows)}",
            flush=True,
        )
    ...
    zenodo_human_jsons = [Path(p).expanduser().resolve() for p in (args.zenodo_human_json or [])]
    ...
    if zenodo_human_jsons:
        sources: List[Path] = []
        if human_json_resolved is not None:
            sources.append(human_json_resolved)
        sources.extend(zenodo_human_jsons)
        merged_human_out = Path(args.out).expanduser().resolve() / "_merged_human_pool.json"
        human_json_resolved, merged_stats = _merge_multiple_human_sources(
            sources=sources,
            seed=int(args.seed),
            out_path=merged_human_out,
            max_total_rows=(None if int(args.human_merge_max_rows) <= 0 else int(args.human_merge_max_rows)),
        )
        merged_stats_path = Path(args.out).expanduser().resolve() / "human_merge_stats.json"
        merged_stats_path.parent.mkdir(parents=True, exist_ok=True)
        merged_stats_path.write_text(json.dumps(merged_stats, indent=2), encoding="utf-8")
        print(
            f"[human-merge] merged {len(sources)} sources -> {human_json_resolved} "
            f"(total={merged_stats['merged_total']})",
            flush=True,
        )
    ...
    if args.shard_size is not None:
        build_sharded_training_dataset(
            ...
            nested_train_val=bool(args.nested_train_val),
            miner_feature_schema=schema_mode,
            extra_bot_jsons=[Path(p).expanduser().resolve() for p in (args.bot_json or [])],
            bot_json_fraction=float(args.bot_json_fraction),
            target_bot_ratio=args.target_bot_ratio,
            log_every_shards=max(0, int(args.log_every_shards)),
