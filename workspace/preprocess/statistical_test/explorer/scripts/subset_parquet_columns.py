...
Examples (repo root):
 ...
 PYTHONPATH=. python workspace/preprocess/statistical_test/explorer/feature_2/scripts/subset_parquet_columns.py \
    --data-dir workspace/dataset/unpreprocessed/train/irc \
    --out-dir workspace/preprocess/statistical_test/explorer/feature_2/data/irc/subset \
    --keep-file workspace/preprocess/statistical_test/explorer/feature_2/config/keep_features.txt
"""
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default=[],
        dest="parquet_inputs",
        help="Single input parquet path (repeatable). Output auto-written under --out-dir using same filename.",
    )
    ap.add_argument(
        "--write-keep-list",
        type=Path,
    ...
    for in_path in args.parquet_inputs:
        ...
        if ref_cols is None:
            ref_cols = list(sub.columns)
        out_p = out_dir / in_p.name
        sub.to_parquet(out_p, index=False)
        print(f"[subset_parquet_columns] {in_p} -> {out_p} cols={len(sub.columns)}", file=sys.stderr)

    if not args.data_dir and not args.parquet_pairs and not args.parquet_inputs:
        raise SystemExit("Provide --data-dir and/or --parquet IN OUT pairs and/or --parquet-input PATH")
